...
namespace OhioMTBTrailStatus.Data.Repositories.Base
{
	...
	public abstract class OhioMTBTrailStatusBaseRepository<T> : SQLBaseRepository, IRepository<T> where T : OhioMTBTrailStatusBase
	{
		private readonly string _tableName;

		public OhioMTBTrailStatusBaseRepository(string tableName)
		{
		...
		public virtual long Add(T item)
		{
			...
			using (IDbConnection cn = OMTSDbConnection())
			{
				cn.Open();
				var insertQuery = Helpers.DynamicQuery.GetInsertQuery(_tableName, parameters);
				var id = (int)cn.ExecuteScalar(insertQuery, parameters);
				item.Id = id;
				return id;
			}
		}
